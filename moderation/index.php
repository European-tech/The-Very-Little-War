...
if (isset($_POST['motdepasseadmin'])) {
	...
	if (!rateLimitCheck($_SERVER['REMOTE_ADDR'], 'moderation_login', RATE_LIMIT_ADMIN_MAX, RATE_LIMIT_ADMIN_WINDOW)) {
		logWarn('MODERATION', 'Moderation login rate limited', ['ip_hash' => substr(hash_hmac('sha256', ($_SERVER['REMOTE_ADDR'] ?? ''), (defined('SECRET_SALT') ? SECRET_SALT : 'tvlw_salt')), 0, 12)]);
	...
	<form action="index.php" method="post">
		...
		<input type="submit" name="valider" value="Valider" />
			...
			if (isset($_POST['verouillersujet'])) {
				...
				withTransaction($base, function() use ($base, $verouillersujet) { // ADMIN-P26-002: atomic lock+cleanup
			...
			if (isset($_POST['deverouillersujet'])) {
				...
				withTransaction($base, function() use ($base, $deverouillersujet) { // ADMIN-P26-006: atomic unlock+cleanup
