...
<h4>Multicomptes — Recherche par IP</h4></div>
<div class="panel-body">
<form method="post" action="ip.php">
    <?php echo csrfField(); ?>
    <input type="text" name="ip" placeholder="Adresse IP" style="width:220px;" />
    <button type="submit">Rechercher</button>
</form>
<?php
// H-002: Read from POST, validate, add CSRF check
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    csrfCheck();
    $ip = isset($_POST['ip']) ? trim($_POST['ip']) : '';
    // Validate: must be a syntactically valid IPv4 or IPv6 address
