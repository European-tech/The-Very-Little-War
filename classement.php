...
if(isset($_POST['joueurRecherche']) AND !empty($_POST['joueurRecherche'])) {
	...
	if($recherche['joueurExiste'] == 1) {
		...
		$playerScore = dbFetchOne($base, 'SELECT a.' . $order . ' AS score FROM autre a JOIN membre m ON m.login = a.login WHERE a.login=? AND m.x != -1000 AND m.estExclu = 0 AND a.login NOT LIKE \'Visiteur%\'', 's', $searchLogin);
		if ($playerScore) {
			// LOW-018: Count distinct score values above the player's score to get DENSE_RANK-style position
			$rankRow = dbFetchOne($base, 'SELECT COUNT(DISTINCT a.' . $order . ') AS rank FROM autre a JOIN membre m ON m.login = a.login WHERE a.' . $order . ' > ? AND m.x != -1000 AND m.estExclu = 0 AND a.login NOT LIKE \'Visiteur%\'', 'd', $playerScore['score']);
...
if(isset($_GET['sub']) AND $_GET['sub'] == 0) {
	...
	if ($mode === 'daily'):
		...
		$dailyPlayers = dbFetchAll($base,
			'SELECT a.login, a.totalPoints, a.pointsAttaque, a.pointsDefense, a.ressourcesPillees, a.tradeVolume, a.victoires, a.points, a.idalliance,
			 ...
			 WHERE m.derniereConnexion >= ? AND m.x != -1000 AND m.estExclu = 0 AND a.login NOT LIKE \'Visiteur%\'
	...
	$nombreDeJoueursParPage = LEADERBOARD_PAGE_SIZE;
		...
		if(isset($_SESSION['login'])) {
			if(isset($pasTrouve) AND $pasTrouve==1) {
				...
				$myScore = dbFetchOne($base, 'SELECT a.' . $order . ' AS score FROM autre a JOIN membre m ON m.login = a.login WHERE a.login=? AND m.x != -1000 AND m.estExclu = 0 AND a.login NOT LIKE \'Visiteur%\'', 's', $_SESSION['login']);
				if ($myScore) {
					// LOW-018: DENSE_RANK — count distinct scores above player's score
			$myRank = dbFetchOne($base, 'SELECT COUNT(DISTINCT a.' . $order . ') AS rank FROM autre a JOIN membre m ON m.login = a.login WHERE a.' . $order . ' > ? AND m.x != -1000 AND m.estExclu = 0 AND a.login NOT LIKE \'Visiteur%\'', 'd', $myScore['score']);
	...
	$retour = dbFetchOne($base, 'SELECT COUNT(*) AS nb_joueurs FROM autre a JOIN membre m ON m.login = a.login WHERE m.x != -1000 AND m.estExclu = 0 AND a.login NOT LIKE \'Visiteur%\'');
	$donnees = $retour;
