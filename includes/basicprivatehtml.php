...
$depot = dbFetchOne($base, 'SELECT * FROM constructions WHERE login = ?', 's', $_SESSION['login']);
...
if($autre['niveaututo'] == 9 and isset($_POST['finir'])){
	...
	dbExecute($base, 'UPDATE autre SET niveaututo = 10 WHERE login = ?', 's', $_SESSION['login']);
}
...
<body class="theme-black" style="font-weight:regular">
  ...
  <div class="panel panel-left panel-cover">
    ...
    <p style="display:block;font-family:magmawave_capsbold;margin-top:-20px;">
        <img src="images/profil/<?php echo htmlspecialchars($autre['image'], ENT_QUOTES, 'UTF-8'); ?>" style="max-width:50px;max-height:50px;vertical-align:middle;margin-left:10px;" alt="profil"/> <a style="color:white" href="joueur.php?id=<?php echo urlencode($_SESSION['login']); ?>"><?php echo htmlspecialchars($_SESSION['login'], ENT_QUOTES, 'UTF-8'); ?></a>
    </p>
    </div>
    <?php
    ...
    debutContent();
      ...
      $nb_rapports_nonlus = dbCount($base, 'SELECT COUNT(*) AS cnt FROM rapports WHERE destinataire = ? AND statut = 0', 's', $_SESSION['login']);
        ...
        $nbSujets = dbFetchOne($base, 'SELECT count(*) AS nbSujets FROM sujets WHERE statut = 0');
		$statutForum = dbFetchOne($base, 'SELECT count(*) AS nbLus FROM statutforum WHERE login = ?', 's', $_SESSION['login']);
    ...
    $exAlliance = dbFetchOne($base, 'SELECT idalliance FROM autre WHERE login = ?', 's', $_SESSION['login']);
	$idalliance = $exAlliance;
	$bonusDuplicateur = 1;
